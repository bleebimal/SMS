...
if(isset($_GET['iface'])) {
	...
	$dbs = $db->prepare('SELECT time,interfaces FROM stats_history WHERE serverid = ? AND time >= ? ORDER BY time DESC');
	...
	$dbs = $db->prepare('SELECT time,memtotal,memused,membuffers FROM stats_history WHERE serverid = ? AND time >= ? ORDER BY time DESC');
	$dbs->execute(array($server->id, $mintime));
	...
	# Set color scheme.
	$data->setPalette('Memory Used', array('R'=>31,'G'=>95,'B'=>0, 'Alpha'=>100));
} else if(isset($_GET['disk'])) {
	$title = 'Disk usage (1 hour)';
	$dbs = $db->prepare('SELECT time,disktotal,diskused,diskfree FROM stats_history WHERE serverid = ? AND time >= ? ORDER BY time DESC');
	$dbs->execute(array($server->id, $mintime));
	...
	$data->setAxisName(0, "Gigabytes");
	# Set color scheme.
	$data->setPalette('Disk Used', array('R'=>0,'G'=>0,'B'=>255,'Alpha'=>100));
}
